...
export function setupFavorites() {
  ...
  function read() {
    try {
      ...
      if (Array.isArray(value))
        saved = new Set(value.filter((id) => typeof id === "string"));
    } catch {
      /* Keep this session's selections if storage is unavailable. */
    }
  }
  read();
  ...
  function refresh() {
    ...
    document.querySelector("[data-saved-empty]").hidden =
      !onlySaved || visible !== 0;
    if (onlySaved)
      document.querySelector("[data-result-count]").textContent =
        visible + " saved listing" + (visible === 1 ? "" : "s");
  }
