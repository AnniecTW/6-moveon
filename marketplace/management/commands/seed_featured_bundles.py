...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        seller, created = get_user_model().objects.get_or_create(
            ...
            defaults={"email": "featured-demo@example.invalid", "display_name": "MoveOn Demo"})
        if created:
        ...
        for scene in SCENES:
            for spec in scene["items"]:
                category, _ = ItemCategory.objects.get_or_create(
                    category_name="Home Decor" if spec["type"] in ("Lamp", "Pillow") else "Furniture")
                ...
                _, created = Listing.objects.get_or_create(
                    seller=seller, title=spec["title"], defaults={
                        "item_type": item_type, "description": "Reference demo item from the " + scene["title"] + ".",
                        ...
                        "bundle_eligible": True, "fulfillment_option": Listing.Fulfillment.PICKUP,
                    })
                added += int(created)
        self.stdout.write(self.style.SUCCESS(f"Added {added} featured demo listings. Existing listings were preserved."))
