...
ORDERING = {"newest": "-created_at", "price-asc": "listing_price",
...
def browse_context(request):
    ...
    if form.is_valid():
        ...
        for name, lookup in [
            ("category", "item_type__category_id__in"), ("item_type", "item_type_id__in"),
        ...
        for name, lookup in [("min_price", "listing_price__gte"), ("max_price", "listing_price__lte")]:
            if values.get(name) is not None:
                listings = listings.filter(**{lookup: values[name]})
        if values.get("sort") == "popular":
            listings = listings.annotate(inquiry_count=Count("conversations", distinct=True)).order_by(
                "-inquiry_count", "-created_at", "pk")
        else:
            listings = listings.order_by(ORDERING.get(values.get("sort"), "-created_at"), "pk")
        for name in ("q", "category", "item_type", "condition", "fulfillment", "min_price", "max_price", "bundle"):
            labels = dict((str(k), str(v)) for k, v in getattr(form.fields[name], "choices", []))
    ...
    return {"listings": [decorate_listing(item) for item in listings],
            "filter_form": form, "filter_chips": chips, "featured_bundles": featured_bundles()}
