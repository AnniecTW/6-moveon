...
from . import messaging as bundle_messaging
...
class BundleStartModalView(LoginRequiredMixin, View):
    ...
    Anonymous visitors are redirected to the campus login screen by
    CampusAccessMiddleware before this view ever runs; LoginRequiredMixin
    stays as defense-in-depth, matching the rest of this app's convention."""
...
class BundleStartView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Invalid category selection."}, status=400
            )
...
class BundleSummaryView(LoginRequiredMixin, View):
    ...
    Checkout delegates to bundles.messaging.send_requests - the same
    idempotent bridge the messaging API exposes at
    /api/messaging/bundles/<id>/send-requests/ - rather than creating
    Conversation/Message rows here. That keeps "assemble a bundle -> notify
    sellers" as a single source of truth with one set of guarantees
    (idempotent per-item requests, buyer/ACTIVE/self-sale checks), called
    from two entry points: this server-rendered no-JS fallback, and the
    JS-driven fetch() in static/js/bundles/checkout.js for the primary,
    no-page-reload path."""
    ...
    def post(self, request, *args, **kwargs):
        bundle = self._get_bundle(request)
        response = bundle_messaging.send_requests(request, bundle_id=bundle.id)

        if response.status_code >= 400:
            payload = json.loads(response.content)
            messages.error(
                request, payload.get("error", "Something went wrong. Please try again.")
            )
            return redirect(f"{reverse('bundles:builder')}?bundle={bundle.id}")
