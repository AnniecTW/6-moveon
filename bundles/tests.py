...
class BundleWizardTests(TestCase):
    ...
    Every user here must be campus-access-eligible (an @illinois.edu email,
    email_verified, email_verified_at set) - CampusAccessMiddleware gates
    every bundles:* route on has_campus_access(), independent of and prior
    to any view-level LoginRequiredMixin, so a plain create_user()+
    force_login() (as the pre-origin/week3 version of these tests used)
    would get redirected to /account/ before any view code runs.
    """
    ...
    def setUpTestData(cls):
        ...
        cls.seller_a = cls.user("seller_a")
        cls.seller_b = cls.user("seller_b")
    ...
    def test_full_wizard_happy_path(self):
        ...
        # Checkout now delegates to bundles.messaging.send_requests instead
        # of hand-rolling Conversation/Message rows in the view itself - the
        # end state (one Conversation + one Message per seller) must match.
    ...
    def test_checkout_error_message_surfaces_from_send_requests(self):
        # BundleSummaryView.post no longer duplicates the minimum-items
        # check itself - it delegates entirely to
        # bundles.messaging.send_requests and translates *its* JSON error
        # into a messages.error(). This confirms that wiring actually
        # surfaces the real error text to the buyer, not just a redirect.
        bundle = self._start_wizard(item_type_ids=[self.lamp_type.id])
        summary_url = f"{reverse('bundles:summary')}?bundle={bundle.id}"

        response = self.client.post(summary_url, follow=True)
        self.assertContains(
            response, "Add at least three items before sending requests."
        )
    ...
    def test_anonymous_user_redirected_to_login(self):
        ...
        # origin/week3 has no "login" URL name - CampusAccessMiddleware
        # redirects anonymous visitors to the campus account/login overlay.
        self.assertIn(reverse("account"), response["Location"])

    def test_non_campus_user_redirected_from_bundle_routes(self):
        # A real account that simply hasn't completed campus email
        # verification yet must be redirected just like a guest -
        # CampusAccessMiddleware gates on has_campus_access(), which
        # LoginRequiredMixin alone (is_authenticated only) would miss.
        unverified = User.objects.create_user(
            username="unverified",
            email="unverified@illinois.edu",
            password="pass12345",
            display_name="Unverified",
        )
        self.client.force_login(unverified)

        for name in ("bundles:select_space", "bundles:start_modal"):
